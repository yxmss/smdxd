...
?>
    <div class="main-bg theme-box radius8 main-shadow author-content">
        <div class="index-tab box-header">
            <ul class="scroll-x mini-scrollbar">
                ...
                <?php echo zib_author_main_tab('nav'); ?>
            </ul>
        </div>
        ...
        <div class="tab-content author-tab-content">
            ...
            <?php echo zib_author_main_tab('content'); ?>
        </div>
    </div>
<?php }
...
function zib_author_main_tab($type = 'nav')
{
    $tabs = array(
        array(
            ...
            'name' => '<i class="fa fa-cog mr6"></i>设置',
            'id' => 'user-data-set',
            'only_me' => true,
        )
    );
...
function zib_get_tab($type = 'nav', $tabs = array(), $nav_class = '', $con_class = 'ajaxpager')
{
    ...
    foreach ($tabs as $tab) {
        ...
        $con .= '<div class="tab-pane fade ' . $con_class . '" id="' . $id . '">
                ...
                <a href="' . $ajax_url . '?type=' . $id . '&id=' . $author_id . '" class="ajax_load ajax-next ajax-open"></a>
                </span>
...
function zib_author_datas($user_id = '', $class = 'box-body', $t_class = 'muted-2-color', $v_class = '')
{
    ...
    $datas = array(
        array(
            ...
            'title' => 'QQ',
            'value' => get_user_meta($user_id, 'qq', true),
            'spare' => '未知',
            'no_show' => true,
        ), array(
            'title' => '微信',
            'value' => get_user_meta($user_id, 'weixin', true),
            'spare' => '未知',
            'no_show' => true,
        ), array(
            'title' => '微博',
            'value' => get_user_meta($user_id, 'weibo', true),
            'spare' => '未知',
            'no_show' => true,
        ), array(
            'title' => 'Github',
            'value' => get_user_meta($user_id, 'github', true),
            'spare' => '未知',
            'no_show' => true,
        )
    );
...
function zib_get_url_link($user_id, $class = 'focus-color')
{
    ...
    return '<a class="' . $class . '" href="' . $user_url . '" target="_blank">' . $url_name . '</a>';
}
...
?>
    ...
    <ul class="list-inline scroll-x mini-scrollbar box-body notop">
        <li class="active"><a class="muted-2-color" data-toggle="tab" href="#author-tab-dataset">个人资料</a></li>
        <?php
        if (_pz('post_rewards_s')) {
            echo '<li class=""><a class="muted-2-color" data-toggle="tab" href="#author-tab-rewards">打赏设置</a></li>';
        }
        ?>
        <li class=""><a class="muted-2-color" data-toggle="tab" href="#author-tab-avatarset">修改头像</a></li>
        <li class=""><a class="muted-2-color" data-toggle="tab" href="#author-tab-coverimgset">修改封面</a></li>
        <li class=""><a class="muted-2-color" data-toggle="tab" href="#author-tab-securityset">账户设置</a></li>
    </ul>
    <div class="box-body notop">
        <div class="tab-content author-tab-set">
            <div class="tab-pane fade in active list-unstyled" id="author-tab-dataset">
                <form>
                    <li>
                        <div class="author-set-left">昵称</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="name" value="<?php echo $_d['nickname'] ?>" placeholder="请输入用户名">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">邮箱</div>
                        <div class="author-set-right">
                            <div data-toggle="tooltip" title="邮箱暂时不可修改，如需修改请联系管理员" type="input" class="form-control" name="email"><?php echo $_d['email'] ?></div>
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">个人签名</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="desc" value="<?php echo $_d['desc'] ?>" placeholder="请简短的介绍自己">
                        </div>
                    </li>
                    <li>
                    ...
                    <li>
                        ...
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="address" value="<?php echo $_d['address'] ?>" placeholder="请输入居住地址">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">个人网站</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="url_name" value="<?php echo $_d['url_name'] ?>" placeholder="请输入网站名称">
                            <input type="input" class="form-control" name="url" style="margin-top:10px" value="<?php echo $_d['url'] ?>" placeholder="请输入网址">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">QQ</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="qq" value="<?php echo $_d['qq'] ?>" placeholder="请输入QQ">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">微信</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="weixin" value="<?php echo $_d['weixin'] ?>" placeholder="请输入微信">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">微博</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="weibo" value="<?php echo $_d['weibo'] ?>" placeholder="请输入微博地址">
                        </div>
                    </li>
                    <li>
                        <div class="author-set-left">Github</div>
                        <div class="author-set-right">
                            <input type="input" class="form-control" name="github" value="<?php echo $_d['github'] ?>" placeholder="请输入Github地址">
                        </div>
                    </li>
                    <li>
            ...
            <?php if (_pz('post_rewards_s')) {
                ...
                if ($weixin) {
                    ...
                    $weixin_img = '<img class="lazyload fit-cover" data-src="' . $weixin[0] . '">';
                }
                if ($alipay) {
                    $alipay = wp_get_attachment_image_src($alipay, 'medium');
                    $alipay_img = '<img class="lazyload fit-cover" data-src="' . $alipay[0] . '">';
                }
            ?>
                        ...
                        <div class="box-body radius8 main-shadow">
                            <div class="box-body rewards-title notop">
                                ...
                                <div class="relative line-form">
                                    <input type="input" class="line-form-input" name="rewards_title" value="<?php echo $rewards_title; ?>" placeholder="文章很赞！支持以下吧">
            ...
            <div class="tab-pane fade" id="author-tab-securityset">
                <div class="box-body">
                    <?php echo zib_oauth_set($user_id) ?>
                    <form>
...
function zib_oauth_set($user_id)
{
    ...
    foreach ($args as $arg) {
        ...
        if (_pz('oauth_' . $type . '_s') && !_pz('social')) {
            ...
            if ($oauth_info) {
                $con .= '<a data-toggle="tooltip" title="解绑' . $name . '帐号" user-id="' . $user_id . '" untying-type="' . $type . '" class="oauth-untying author-minicard but ' . $class . ' hollow"><i class="fa ' . $icon . '"></i>已绑定' . $name . ' ' . $oauth_info[$name_key] . '</a>';;
            } else {
                $con .= '<a title="绑定' . $name . '帐号" href="' . esc_url(home_url('/oauth/' . $type . '/')) . '" class="author-minicard but ' . $class . ' hollow"><i class="fa ' . $icon . '"></i>绑定' . $name . '帐号</a>';
            }
        }
    }
                ...
                <dl>
                    ...
                    <?php if ($args['show_button']) { ?>
                        <div class="more-button box-body nobottom">
                            ...
                            ?>
                            <?php echo '<a class="but jb-blue" href="' . get_author_posts_url($user_id) . '">个人中心</a>'; ?>
                        </div>
                    <?php } ?>
                </dl>
            </li>
