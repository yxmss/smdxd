...
function zib_get_comments_list($comment, $depth = 0)
{
	...
	$con =  preg_replace('/\[img=(.*?)\]/', '<img class="box-img lazyload" src="$1">', convert_smilies(get_comment_text()));
	...
	if ($comment->comment_approved !== '0' && $depth) {
		$replyText = get_comment_reply_link(array('add_below' => 'div-comment', 'reply_text' => '回复', 'depth' => $depth, 'max_depth' => $max_depth));
