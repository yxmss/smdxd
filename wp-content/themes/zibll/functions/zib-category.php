...
<div class="page-cover theme-box radius8 main-shadow">
    <img class="lazyload fit-cover"
        <?php echo _pz('lazy_cover', true) ? 'src="' . $src . '" data-src="' . $img . '"' : 'src="' . $img . '"'; ?>>
    ...
    if ($cats) {
        foreach ($cats as $key => $value) {
            if ($value) {
                ...
                $links .= $before . '<a ajax-replace="true" class="' . $link_class . '" title="查看此专题更多文章" href="' . get_category_link($cat->term_id) . '">' . $cat->name . ($shou_count ? ' ' . $cat->count : '') . '</a>' . $after;
            }
        }
    }
    return $links;
