...
function zib_posts_mini_while($args = array(), $number)
{
    ...
    if ($args['show_number']) {
        ...
        $title = '<b class="number-style ' . (!empty($cls[$number - 1]) ? $cls[$number - 1] : '') . '" href="' . get_permalink() . '">' . $number . '</b>' . $title;
    }
    $lists_class = 'posts-mini';
    $title_l = '<div class="item-heading' . ($args['show_thumb'] ? ' text-ellipsis-2' : ' text-ellipsis') . '">
            <a' . _post_target_blank() . ' href="' . get_permalink() . '">' . $title . '</a>
            </div>
            ';
    $time_ago = zib_get_time_ago(get_the_time('U'));
