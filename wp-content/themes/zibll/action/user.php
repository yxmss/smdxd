...
switch ($_POST['action']) {
    ...
    print_r(json_encode(array('error' => 0, 'ys' => '', 'msg' => '已解除绑定')));
    ...
    case 'set.rewards':
        if (!wp_verify_nonce($_POST['upload_rewards_nonce'], 'upload_rewards')) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
            exit();
        }
        ...
        if (empty($_FILES['weixin']) && empty($_FILES['alipay']) && !$weixin_lao_id && !$alipay_lao_id) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请选择收款二维码')));
            exit();
        }
        ...
        print_r(json_encode(array('error' => 0,'msg' => '设置成功', 'weixin_url' => $weixin_url[0],'alipay_url' => $alipay_url[0], 'weixin_img_id' => $weixin_img_id, 'alipay_img_id' => $alipay_img_id,'_POST' =>$_POST , '_FILES' =>$_FILES )));
    ...
    case 'password.edit':
        ...
        if ( empty($_POST['password']) || empty($_POST['password2'])) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '密码不能为空')));
            exit();
        }

        if (strlen($_POST['password']) < 6) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '密码至少6位')));
            exit();
        }

        if ($_POST['password'] !== $_POST['password2']) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '两次密码输入不一致')));
            exit();
        }
        ...
        if(!empty($_POST['oauth_new'])){
            ...
            if($_POST['oauth_new'] != $oauth_new){
                print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败')));
                exit();
            }
            ...
            if (is_wp_error($status)) {
                print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '修改失败，请稍后再试')));
                exit();
            }
            delete_user_meta($cuid, 'oauth_new');
            print_r(json_encode(array('error' => 0, 'msg' => '修改成功，下次请使用新密码登录')));
            exit();
        }
        ...
        if ($_POST['passwordold'] == $_POST['password']) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '新密码和原密码不能相同')));
            exit();
        }
        ...
        if (!$wp_hasher->CheckPassword($_POST['passwordold'], $current_user->user_pass)) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '原密码错误')));
            exit();
        }
    ...
    case 'upload.cover':
        ...
        if (empty($_FILES['file'])) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请选择图像')));
            exit();
        }
        ...
        print_r(json_encode(array('error' => 0, 'msg' => '封面图修改成功', 'img_id' => $img_id, 'img_url' => $image_url[0])));
        exit();
        break;
    ...
    case 'upload.avatar':
        ...
        print_r(json_encode(array('error' => 0, 'msg' => '头像修改成功', 'img_id' => $img_id, 'img_url' => $image_url[0])));
        exit();
        break;
    ...
    case 'edit.datas':
        ...
        if (empty($_POST['name']) || (!empty($_POST['name']) && _new_strlen($_POST['name']) > 12) || (!empty($_POST['name']) && _new_strlen($_POST['name']) < 2)) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '昵称不能为空且限制在2-12字内')));
            exit();
        }

        if (empty($_POST['desc']) || _new_strlen($_POST['desc']) < 4) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请填写签名且不少于5个字符')));
            exit();
        }

        if (is_disable_username($_POST['name'])) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '昵称含保留或非法字符')));
            exit();
        }
        ...
        if(_pz('no_repetition_name',true)){
            ...
            if ($db_name && $db_name!=$cuid ) {
                print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '昵称已经存在，请换一个试试')));
                exit();
            }
        }

        if ($_POST['url'] && ( !zib_is_url($_POST['url']) )) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '网址格式错误')));
            exit();
        }

        if ($_POST['url'] && !$_POST['url_name']) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请输入个人网站名称')));
            exit();
        }

        if ($_POST['url_name'] && !$_POST['url']) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请输入个人网站链接')));
            exit();
        }

        if ($_POST['address'] && _new_strlen($_POST['address']) > 50) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '居住地格式错误')));
            exit();
        }

        if ($_POST['weibo'] && (!zib_is_url($_POST['weibo']) || _new_strlen($_POST['weibo']) > 100)) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '微博格式错误')));
            exit();
        }

        if ($_POST['github'] && (!zib_is_url($_POST['github']) || _new_strlen($_POST['github']) > 100)) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => 'GitHub格式错误')));
            exit();
        }

        if ($_POST['qq'] && !preg_match("/^[1-9]\d{4,16}$/", $_POST['qq'])) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => 'QQ格式错误')));
            exit();
        }

        if ($_POST['weixin'] && _new_strlen($_POST['weixin']) > 30) {
            print_r(json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '微信字数过长，限制在30字内')));
            exit();
        }
