						...
						if (e) {
							if ($("#user_yz_canvas1").length) {
								var r = o.canvas_yz.toLowerCase(),
									a = show_num2.join("");
								if ("" == r) return notyf("请输入验证码", "danger");
								if (r !== a) return notyf("验证码错误！请重新输入", "danger"), void draw(name2, show_num2);
							}
							...
							if ($("#user_yz_canvas1").length) {
								var r = o.canvas_yz.toLowerCase(),
									a = show_num1.join("");
								if ("" == r) return notyf("请输入验证码", "danger");
								if (r !== a) return notyf("验证码错误！请重新输入！", "danger"), void draw(name1, show_num1);
							}
						}
