...
//熊掌号原创文章
$_on = '';
if (_pz('xzh_on') && _pz('xzh_appid') && _pz('xzh_post_on') && _pz('xzh_post_token')) {
    $_on = true;
}
$postmeta_xzh = array(
    array(
...
if (_pz('xzh_on') && _pz('xzh_appid') && _pz('xzh_post_token')) {
...
function hui_postmeta_xzh()
{
    ...
    foreach ($postmeta_xzh as $meta_box) {
        ...
        if ($meta_box_value == "")
            $meta_box_value = $meta_box['std'];
        echo '<span style="margin:15px 20px 15px 0; display:inline-block;"><label><input ' . ($meta_box['disabled'] ? 'disabled="disabled" ' : '') . ($meta_box_value ? 'checked' : '') . ' type="checkbox" value="1" name="' . $meta_box['name'] . '"> ' . (isset($meta_box['title']) ? $meta_box['title'] : '') . '</label></span>';
    }
...
{
    global $post;
    $tskg = get_post_meta($post->ID, 'xzh_post_ison', true);
    if (_pz('xzh_on') && _pz('xzh_appid') && ($tskg || _pz('xzh_post_on')) && _pz('xzh_post_token')) {
        //update_post_meta($post->ID, 'xzh_tui_back', $tskg.'/yes/'.get_post_meta($post->ID, 'is_original', true));
        ...
        if ('publish' == $post->post_status && $plink && strstr($ok, '成功') == false) {
            $type =  _pz('xzh_post_type') ? _pz('xzh_post_type') : 'batch';
            $urls = array();
            $urls[] = $plink;
            $api = 'http://data.zz.baidu.com/urls?appid=' . _pz('xzh_appid') . '&token=' . _pz('xzh_post_token') . '&type=' . $type;
            $ch = curl_init();
            $options =  array(
                CURLOPT_URL => $api,
                CURLOPT_POST => true,
            ...
            $result_text = '推送成功：' . json_encode($result);
            if ($result->error) {
                $result_text = '推送失败：' . json_encode($result);
            }
