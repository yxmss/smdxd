...
require get_theme_file_path('/framework/options-framework.php');
...
function optionsframework_options()
{
	...
	foreach ($options_categories_obj as $category) {
		$options_categories[$category->cat_ID] = rtrim(get_category_parents($category->cat_ID, false, '>'), '>') . ' [共' . $category->count . '篇]';
	}
...
$options[] = array(
	...
	$options[] = array(
		...
		'class' => 'op-multicheck',
		'desc' => __('图片分享备用图片'),
		'question' => '当文章没有任何图片时显示此图片，建议尺寸700*500',
		'std' => $imagepath . 'share_img.jpg',
		'type' => 'upload'
	);
...
*/
	...
	$options[] = array(
		'name' => '推送密钥 token',
		'id' => 'xzh_post_token',
		'desc' => '密钥获取：https://ziyuan.baidu.com/ydzq/includeday',
		'std' => '',
		'type' => 'text'
	);

	$options[] = array(
		'name' => __('所有文章实时推送', 'zib_language'),
		'id' => 'xzh_post_on',
		'std' => false,
		'desc' => '开启',
		'question' => '使用此功能，你还需要开启本页中的 百度熊掌号 和 AppID 以及 token 的设置',
		'type' => 'checkbox'
	);
	...
	$options[] = array(
		...
		'id' => 'xzh_post_type',
		'std' => 'batch',
		'desc' => '如果天级收录日配额较低，建议选择周级收录',
		'type' => "radio",
		'options' => array(
			'batch' => __('站点周级收录', 'zib_language'),
			'realtime' => __('站点天级收录', 'zib_language'),
		)
	);
	...
	$options[] = array(
		...
		'class' => 'op-multicheck',
		'type' => "",
		'desc' => 'WordPress配置SMTP邮箱，解决邮件发送问题。功能和SMTP插件一致，所以！不能和其他SMTP插件一起开启！建议用QQ邮箱，注意QQ邮箱的密码是独立密码'
	);
