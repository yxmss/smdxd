...
class Options_Framework_Interface
{
	...
	static function optionsframework_tabs()
	{
		...
		foreach ($options as $value) {
			...
			if ($value['type'] == "heading") {
				...
				$menu .= '<a id="options-group-' .  $counter . '-tab" class="qnav-tab ' . $class . '" title="' . esc_attr($value['name']) . '" href="' . esc_attr('#options-group-' .  $counter) . '">' . esc_html($value['name']) . '</a>';
			}
		}
		return $menu;
	}
	...
	static function optionsframework_fields()
	{
		...
		foreach ($options as $value) {
			...
			if (function_exists('framework_option_args') && !empty(framework_option_args()['debug']) && isset($value['id'])) {
				$explain_value .= ' - ID: ' . $value['id'];
			}
			...
			switch ($value['type']) {
				...
				case 'number':
					...
					if (isset($value['settings']['min'])) {
						$_min = $value['settings']['min'];
						if (is_numeric($_min)) {
							$min = $_min;
						}
					};
					if (isset($value['settings']['max'])) {
						$_max = $value['settings']['max'];
						if (is_numeric($_min)) {
							$max = $_max;
						}
					};
					if (isset($value['settings']['step'])) {
						$_step = $value['settings']['step'];
						if (is_numeric($_step)) {
							$step = $_step;
						}
					};
					if (isset($value['settings']['prefix'])) {
						$prefix = $value['settings']['prefix'];
					};
					if (isset($value['settings']['postfix'])) {
						$postfix = $value['settings']['postfix'];
					};

					$output .= '<input id="' . esc_attr($value['id']) . '" data-min="' . $min . '" data-max="' . $max . '" data-from="' . esc_attr($val) . '" data-step="' . $step . '" data-prefix="' . $prefix . '" data-postfix="' . $postfix . '" class="number-slider of-input" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '"   type="text" value="' . esc_attr($val) . '" />';
					break;

				case 'html-src':

					$output .= '<div data-html-src="'.$value['src'].'" id="' . esc_attr($value['id']) . '"></div>';
					break;
				...
				case "colorradio":
					$name = $option_name . '[' . $value['id'] . ']';
					foreach ($value['options'] as $key => $valu) {
						$selected = '';
						$checked = '';
						if ($val != '') {
							if ($val == $key) {
						...
						$output .= '<div class="of-radio-img-label">' . esc_html($key) . '</div>';
						$output .= '<a style="background-color:#' . $valu . ';" href="javascript:;" class="of-radio-img-img of-radio-color' . $selected . '" onclick="document.getElementById(\'' . esc_attr($value['id'] . '_' . $key) . '\').checked=true;"></a>';
						// $output .= '<img src="' . esc_url( $option ) . '" alt="' . $option .'" class="of-radio-img-img' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;" />';
					}
					break;
				...
				case "checkbox":
					...
					if ($question_value) {
						$output .= '<div class="question"><span class="dashicons dashicons-editor-help"></span><div class="question_nr">' . $question_value . '</div></div>' . "\n";
					}
			...
			if (($value['type'] != "heading") && ($value['type'] != "info")) {
				...
				if (($value['type'] != "checkbox") && ($value['type'] != "editor")) {
					$output .= '<div class="explain">' . wp_kses($explain_value, $allowedtags) . "\n";
					if ($question_value) {
						$output .= '<div class="question"><span class="dashicons dashicons-editor-help"></span><div class="question_nr">' . $question_value . '</div></div>' . "\n";
					}
					$output .= '</div>' . "\n";
